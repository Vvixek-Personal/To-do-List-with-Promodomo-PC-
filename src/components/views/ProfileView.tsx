...
export function ProfileView({
  ...
  return (
    <div className="w-full max-w-3xl mx-auto flex flex-col gap-6 font-sans">
      ...
      <div className="rounded-3xl border border-white/80 bg-white/70 backdrop-blur-2xl p-7 sm:p-8 shadow-[0_14px_40px_rgba(30,40,90,0.06),inset_0_1px_2px_rgba(255,255,255,0.95)] flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
        <div className="relative">
          {user?.photoURL ? (
            ...
            <div className="w-20 h-20 rounded-2xl border-2 border-white bg-gradient-to-tr from-[#3b5bfd] to-[#6366f1] shadow-[0_8px_20px_rgba(59,91,253,0.35)] flex items-center justify-center">
              <User className="w-10 h-10 text-white stroke-[1.8]" />
            </div>
          )}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2.5 justify-center sm:justify-start">
            <h2 className="text-2xl font-bold text-[#0f172a] truncate">
              {user?.displayName || (user ? 'Focus Explorer' : 'Guest Explorer')}
            </h2>
            <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold uppercase bg-blue-100 text-[#1d4ed8] w-fit mx-auto sm:mx-0 flex-shrink-0">
              <Zap className="w-3.5 h-3.5 fill-[#3b5bfd] text-[#3b5bfd]" />
              {user ? 'Cloud Sync' : 'Guest'}
            </span>
          </div>

          <p className="text-xs text-[#64748b] mt-1 font-medium truncate">
            {user?.email ? user.email : 'Local Storage • Guest Mode'}
          </p>

          <div className="flex items-center justify-center sm:justify-start gap-4 mt-4 text-xs font-semibold text-[#475569]">
            <div>
              <span className="font-bold text-[#0f172a] text-sm">{completedPomodorosCount}</span> Poms Completed
            </div>
            <div>&bull;</div>
            <div>
              <span className="font-bold text-[#0f172a] text-sm">{streakDays}</span> {streakDays === 1 ? 'Day' : 'Days'} Streak
            </div>
          </div>
        </div>
        ...
        {user && (
          <div className="flex-shrink-0">
            <button
              ...
              disabled={loading}
              className="flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-white/80 hover:bg-white border border-[#e2e8f0] text-xs font-semibold text-[#475569] hover:text-rose-600 shadow-sm transition-all cursor-pointer"
            >
      ...
      {error && (
        <div className="flex items-center gap-2 p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-700 text-xs font-medium">
        ...
        <div className="rounded-3xl border border-white/90 bg-gradient-to-br from-white/95 via-blue-50/40 to-indigo-50/60 backdrop-blur-2xl p-7 sm:p-8 shadow-[0_16px_45px_rgba(30,40,90,0.08),inset_0_1px_2px_rgba(255,255,255,0.95)]">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="max-w-xl">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold tracking-wider uppercase bg-blue-100 text-[#3b5bfd] border border-blue-200/60">
                <Sparkles className="w-3.5 h-3.5" /> Google Integration
              </span>
              <h3 className="text-xl sm:text-2xl font-bold text-[#0f172a] tracking-tight mt-2.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-5">
                <div className="flex items-start gap-2.5 p-3 rounded-2xl bg-white/80 border border-white/90 shadow-sm">
                  ...
                  <div>
                    <div className="text-xs font-bold text-[#0f172a]">Real-Time Cloud Sync</div>
                    <div className="text-[11px] text-[#64748b]">Access your tasks on your laptop, phone, or tablet.</div>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2.5 p-3 rounded-2xl bg-white/80 border border-white/90 shadow-sm">
                  ...
                  <div>
                    <div className="text-xs font-bold text-[#0f172a]">Automatic Safe Backup</div>
                    <div className="text-[11px] text-[#64748b]">Never lose tasks if you clear browser data.</div>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2.5 p-3 rounded-2xl bg-white/80 border border-white/90 shadow-sm">
                  ...
                  <div>
                    <div className="text-xs font-bold text-[#0f172a]">Cross-Device Badges</div>
                    <div className="text-[11px] text-[#64748b]">Preserve your focus streaks & milestones.</div>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2.5 p-3 rounded-2xl bg-white/80 border border-white/90 shadow-sm">
                  ...
                  <div>
                    <div className="text-xs font-bold text-[#0f172a]">1-Click Instant Sign-In</div>
                    <div className="text-[11px] text-[#64748b]">Fast, secure authentication with Google.</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center sm:items-start lg:items-end justify-center flex-shrink-0">
              <button
                ...
                disabled={loading}
                className="flex items-center gap-3 px-6 py-3.5 rounded-2xl bg-white hover:bg-slate-50 border border-[#cbd5e1] text-xs sm:text-sm font-bold text-[#0f172a] shadow-[0_4px_16px_rgba(0,0,0,0.06)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.1)] transition-all cursor-pointer transform hover:-translate-y-0.5 active:translate-y-0 whitespace-nowrap"
              >
              ...
              <span className="text-[11px] text-[#64748b] mt-2 font-medium">Free • Instant cross-device sync</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="rounded-3xl border border-white/80 bg-white/70 backdrop-blur-2xl p-7 shadow-[0_14px_40px_rgba(30,40,90,0.06),inset_0_1px_2px_rgba(255,255,255,0.95)] space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {badges.map((badge, idx) => (
            ...
            >
              ...
              <div className="text-[10px] text-[#64748b] mt-0.5 font-medium">{badge.description}</div>
              <div className="mt-2">
        ...
        <div className="pt-4 border-t border-[#e2e8f0] flex items-center justify-between text-xs text-[#64748b] font-medium">
          ...
          <span className="text-emerald-700 bg-emerald-100/80 px-2.5 py-1 rounded-full font-bold text-[11px] flex items-center gap-1">
            {user ? (
              <>
